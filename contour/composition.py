...
def pitches_to_notes(pitches, offset=0):
    ...
    for pitch in pitches:
        p_class, octave = _utils.octave_calculator(pitch)
        octave = octave + offset
        result.append(_utils.notes[p_class] + str(octave))
...
def cseg_pitch_sets_to_music21(cseg, pitch_set, octave=4):
    ...
    pitch_numbers = _utils.absolute_pitches_permutation(cseg, pitch_set)
    pitch_numbers = _utils.flatten(pitch_numbers)
